 ...
 * ARP implementation.
 */
class INET_API SecureArp : public OperationalBase, public IArp
{
  public:
    class ArpCacheEntry;
    ...
    {
      public:
        SecureArp *owner = nullptr;    // owner ARP module of this cache entry
  ...
  protected:
    ...
    void genRSAKey();
    string signRSA(string mac);
    bool verifyRSA(const Ptr<const ArpPacket>& arpReply);

    void genECDSAKey();
    string signECDSA(string mac);
    bool verifyECDSA(const Ptr<const ArpPacket>& arpReply);
};
